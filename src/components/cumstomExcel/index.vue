...
export default {
  ...
  methods: {
    ...
    updataSheet(data) {
      window.luckysheet.destroy()
      const { info, sheets, celldata } = data;
      const option = {
        ...this.option,
        ...info,
    ...
    cellUpdated() {
      const [r, c, oldVal, newVal, isRefresh] = arguments;

      if (isRefresh) {
        console.log('cellUpdated: ', arguments);
        this.$emit('sheetUpdata', arguments)
      }

    },
    // range渲染比 一个一个单元格效率高很多
    setRangeVal(data, cb) {
      setRangeVal(data, cb)
    },
    convertToSheetData(data) {
      return convertToSheetData(data)
    }
